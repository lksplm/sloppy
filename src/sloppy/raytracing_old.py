...
def find_eigenray_mpe_debug(elements, ray0, lr=0.03, Niter=50, Nmpe=5, tol=1e-9, **kwargs):
    ...
    alltraj = []
    for i in range(Nmpe):
        rconv, rseq, tols = _find_eigenray_formpe(elements, rnew, lr=lr, maxiter=Niter, tol=tol*1e-1, **kwargs)
        alltraj.append(rseq)
        alltols.append(tols)
        if rseq.shape[0]<4:
            rnew = rconv
...
def extract_ABCD_fd(elements, screen, epsr = 1e-1, epss = 1e-1, Nrt=1):
    ...
    """
    mue = np.array([[0, 0, 0, 0], [epsr, 0, 0, 0], [0, epsr, 0, 0], [0, 0, epss, 0], [0, 0, 0, epss]], dtype=np.float)
    guideray = screen.eigenvectors_to_rays(mue)
    trajgr = propagate_system(elements, guideray, Nrt=Nrt, clip=True)
    trajfd = trajgr[-1,:,1:,:]-(trajgr[-1,:,0,:])[:,None,:]
    Mfd = screen.rays_to_eigenvectors(trajgr[-1,:,1:,:]) - screen.rays_to_eigenvectors(trajgr[-1,:,0,:][:,None,:])  
    Mfd = Mfd.T
    Mfd[:2, :] /= epsr
    Mfd[2:, :] /= epss
    return Mfd

def _extract_ABCD_fd(elements, x0, n0, epsr = 1e-1, epss = 1e-1, Nrt=1):
    """Extract an ABCD matrix from raytracing.
    Five guiding rays (of the form (eps, 0, 0, 0)...) are generated 
    and the matrix entries are determined by finite difference.
    Args:
...
def _eigenvector_to_ray(mu, x0, k):
